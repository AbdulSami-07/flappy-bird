...
GAME_SOUNDS = {}
PLAYER = [
    'gallery/sprites/bird-1.png','gallery/sprites/bird-2.png','gallery/sprites/bird-3.png','gallery/sprites/bird-4.png'
]
PLAYER_NO = 0
...
def welcomeScreen():
    ...
    playery = int((SCREENHEIGHT - GAME_SPRITES['player'][0].get_height())/2 + 50)
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
                ...
                if PLAYER_NO >=0:
                    SCREEN.blit(GAME_SPRITES['player'][PLAYER_NO],(playerx,playery))
                    PLAYER_NO+=1
                    if PLAYER_NO == 4:
                        PLAYER_NO = 0
...
def isCollide(playerx, playery, upperPipes, lowerPipes):
    ...
    for pipe in lowerPipes:
        if (playery + GAME_SPRITES['player'][0].get_height() > pipe['y']) and abs(playerx - pipe['x'] -10) < GAME_SPRITES['pipe'][0].get_width():
            GAME_SOUNDS['hit'].play()
            return True
...
if __name__ == "__main__":
    ...
    GAME_SPRITES['player'] = [
        ...
        pygame.image.load(PLAYER[3]).convert_alpha()
    ]
